    // Renders map upon recieving post data
    // Format expected - compliant with post table schema
    
    
    var renderMapData = function(divtag, data){
            console.log("in renderMapData");
            console.log(data);
            ...
            // Need to retrieve 1.number of posts per nbrhood, 2.sentiment scores per nbrhood
            data.forEach(element => {
                Nbrhoods[`${element.neighbourhood}`]["num_posts"]++;
                Nbrhoods[`${element.neighbourhood}`]["sentiment"] += element.sent_score;
            })

            maxNbrhoodPosts = 0;
            for (elem in Nbrhoods) {
                console.log(elem);
                if (Nbrhoods[elem].num_posts > maxNbrhoodPosts) {
                    maxNbrhoodPosts = Nbrhoods[elem].num_posts;
                    console.log(maxNbrhoodPosts);
                }
            }
            ...
            projection
                ...
                var map = d3.select(`#${divtag}`).append("svg")
                    .attr("width", `${width}px`)
                ...
                console.log("Right before color stuff");
                map.selectAll("path")
                    ...
                    .style("fill", function(d){
                        if (Nbrhoods[`${d.properties.Name}`]["num_posts"] != 0) {
                            return colors(Nbrhoods[`${d.properties.Name}`]["num_posts"]/maxNbrhoodPosts);
                        } else {
                            return "white";
                        }
                    })
                ...
                console.log("Right after color stuff");
                map.append("circle")
                    .attr('cx', 120)
                    .attr('cy', 85)
                    .attr('r','10px')
